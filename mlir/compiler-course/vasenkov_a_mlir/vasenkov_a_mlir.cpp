    ...
    int analyzeNestedDepth(mlir::Region &region) {
        ...
        for (auto &block : region) {
            for (auto &operation : block) {
                deepestLevel = std::max(deepestLevel, evaluateOperationDepth(operation));
            }
        }
        return deepestLevel;
    }
...
public:
    MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(ExamplePass_VasenkovAndrey_FIIT1_MLIR)
