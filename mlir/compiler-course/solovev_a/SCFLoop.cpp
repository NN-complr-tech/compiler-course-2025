...
namespace {
    class ScfForLoopsPass : public PassWrapper<ScfForLoopsPass, OperationPass<ModuleOp>> {
    public:
        StringRef getArgument() const final {
            return "Lab_4_mlir_Solovev_a_FIIT1_MLIR";
        }
        StringRef getDescription() const final { return "Annotate scf.for loops with trip_count attribute if known"; }

        void runOnOperation() override {
            ModuleOp moduleOp = getOperation();

            moduleOp.walk([&](scf::ForOp forOp) {
                auto lowerBound = forOp.getLowerBound().getDefiningOp<arith::ConstantIndexOp>();
                auto upperBound = forOp.getUpperBound().getDefiningOp<arith::ConstantIndexOp>();
