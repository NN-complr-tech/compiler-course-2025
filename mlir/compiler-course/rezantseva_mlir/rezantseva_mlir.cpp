...
class MaxDepthPass : public PassWrapper<MaxDepthPass, OperationPass<func::FuncOp>> {
private:

  bool isLoopOp(Operation* op) {
    return isa<affine::AffineForOp, scf::ForOp, scf::WhileOp, scf::ForallOp, spirv::LoopOp>(op);
  }
 
  bool isNestingOp(Operation* op) {
    return isa<affine::AffineForOp, scf::ForOp, scf::WhileOp, scf::ForallOp,
               spirv::LoopOp, affine::AffineIfOp, scf::IfOp, spirv::SelectionOp>(op);
  }
    ...
    for (Region& region : op->getRegions()) {
      for (Block& block : region) {
        for (Operation& nestedOp : block) {
          maxDepth = std::max(maxDepth, computeDepth(&nestedOp, currentDepth + (isNestingOp(op) ? 1 : 0)));
        }
      }
    }
...
public:
  ...
  if (loopDepths.empty()) {
    ...
    for (unsigned depth : loopDepths) {
      depthAttrs.push_back(IntegerAttr::get(IntegerType::get(func.getContext(), 64), depth));
    }
    func->setAttr("my_loop_depths", ArrayAttr::get(func.getContext(), depthAttrs));
  }
  }
};
