...
private:
    bool processFAddInstruction(llvm::Instruction* Inst) {
        ...
        for (unsigned operandIndex = 0; operandIndex < 2; ++operandIndex) {
            ...
            auto* multiplyOperation = llvm::dyn_cast<llvm::BinaryOperator>(currentOperand);

            if (multiplyOperation && multiplyOperation->getOpcode() == llvm::Instruction::FMul) {
                if (performFusionTransform(addOperation, multiplyOperation, operandIndex))
                    return true;
            }
        }
        return false;
    ...
    bool performFusionTransform(llvm::BinaryOperator* addOp, llvm::BinaryOperator* mulOp, unsigned index) {
        ...
        return generateFMAInstruction(addOp, mulOp, mulOperand1, mulOperand2, addOperand);
    }
    ...
    bool verifyTypeConsistency(llvm::BinaryOperator* baseOp, llvm::Value* val1, 
        ...
        return baseType == val1->getType() &&
               baseType == val2->getType() &&
               baseType == val3->getType();
    }

    bool generateFMAInstruction(llvm::BinaryOperator* addOp, llvm::BinaryOperator* mulOp,
...
public:
    llvm::PreservedAnalyses run(llvm::Function &function,
        ...
        for (llvm::BasicBlock &basicBlock : function) {
            for (llvm::Instruction &instruction : llvm::make_early_inc_range(basicBlock)) {
    ...
    return {
        ...
        [](llvm::PassBuilder &passBuilder) {
            passBuilder.registerPipelineParsingCallback(
                [](llvm::StringRef passName, llvm::FunctionPassManager &functionPM,
