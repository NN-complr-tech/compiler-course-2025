        ...
        llvm::PreservedAnalyses run(llvm::Function& Func, llvm::FunctionAnalysisManager&) {
            ...
            for (auto& Block : Func) {
                ...
                for (auto& Inst : Block) {
                    ...
                    if (BinOp->getOpcode() != llvm::Instruction::SDiv && BinOp->getOpcode() != llvm::Instruction::UDiv)
                        continue;

                    auto* ConstVal = llvm::dyn_cast<llvm::ConstantInt>(BinOp->getOperand(1));
                    ...
                    if (divisor == 1 || divisor == -1) {
                        llvm::Value* NewVal = (divisor == 1) ? Dividend : Builder.CreateNeg(Dividend);
                    ...
                    if ((std::abs(divisor) & (std::abs(divisor) - 1)) == 0) {
                        ...
                        llvm::Value* Shifted = Builder.CreateAShr(Dividend, llvm::ConstantInt::get(BinOp->getType(), ShiftAmount));
