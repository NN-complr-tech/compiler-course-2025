...
namespace {
    ...
    public:
        ...
        bool VisitCXXConstructExpr(clang::CXXConstructExpr* Ctor) {
            ...
            CastInfo.emplace_back(FunctionToCheck, std::move(FromStr), std::move(ToStr));
            return true;
        }
        ...
        bool VisitImplicitCastExpr(clang::ImplicitCastExpr* Cast) {
            ...
            if (const auto* SubExpr = Cast->getSubExpr()) {
                ...
                if (!FromType.isNull() && !ToType.isNull() && FromType.getTypePtr() != ToType.getTypePtr()) {
                    CastInfo.emplace_back(FunctionToCheck, FromType.getAsString(), ToType.getAsString());
                }
            }
            return true;
        }

        std::string getCastDescription(const std::string& from, const std::string& to) {
            return from + " -> " + to;
        }
        ...
        void CastsResult() {
            ...
            for (const auto& Entry : CastInfo) {
                ...
                llvm::outs() << getCastDescription(std::get<1>(Entry), std::get<2>(Entry)) << ": 1\n";
            }
