		...
		bool VisitCXXRecordDecl(const clang::CXXRecordDecl* record) {
			...
			if (record->methods().empty()) {
				...
				for (const auto* method : record->methods()) {
					...
					llvm::interleaveComma(paramTypes, oss, [&](const std::string& type) {
						return type;
						});
					oss << ")";
...
static clang::FrontendPluginRegistry::Add<::LabPluginAction> X("LabPlugin_KolodkinGrigorii_FIIT3_ClangAST", "Prints info about user defined data types");
