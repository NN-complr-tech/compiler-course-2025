// RUN: %clang_cc1 -load
// %llvmshlibdir/ClangAST_1_MamaevaO_FIIT3_ClangAST%pluginext -plugin
// ClangAST_1_MamaevaO_FIIT3_ClangAST -fsyntax-only %s 2>&1 | FileCheck
// --match-full-lines %s
...
static double sum(int a, float b) { return a + b; }
...
static int mul(float a, float b) { return a + sum(a, b); }
...
static void convertTypes() {
  ...
  [[maybe_unused]] IntType int_var =
      float_var; // Переменная не используется, но оставлена для будущего
                 // использования.
}
...
static void handlePointers() {
  int value;
  [[maybe_unused]] void *void_ptr =
      &value; // Переменная не используется, но оставлена для будущего
              // использования.
}
...
static void convertToBool() {
  int number = 7;
  [[maybe_unused]] bool boolean_value =
      number; // Переменная не используется, но оставлена для будущего
              // использования.
}
...
static void enumConversions() {
  ...
  [[maybe_unused]] Status status =
      static_cast<Status>(color_code); // int -> Status
}
